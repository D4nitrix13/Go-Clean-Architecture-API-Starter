...
.PHONY: run
run: ## run the API server locally (Go on host)
...
.PHONY: db-start
db-start: ## start the database server via docker compose
	@mkdir -p testdata/postgres
	@$(DC) up -d db

.PHONY: db-stop
db-stop: ## stop the database server via docker compose
	@$(DC) stop db || true

.PHONY: dc-up
dc-up: ## start API server and database via docker compose
	@mkdir -p testdata/postgres
	@$(DC) up -d

.PHONY: dc-down
dc-down: ## stop API server and database via docker compose
	@$(DC) down
...
testdata: ## populate the database with test data
	...
	@$(DC) exec -T db psql -U postgres -d go_restful -f /testdata/testdata.sql
...
.PHONY: up
up: ## Levanta DB, aplica migraciones/testdata y arranca la API
	@echo "==> Levantando Base de Datos..."
	make db-start

	@echo "==> Ejecutando migraciones + test data..."
	make testdata
...
.PHONY: down
down: ## stop all docker compose services (safe down)
